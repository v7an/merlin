...
export const name: string = "banners";

export const perm: number = 1;

export const slashCommand: any = { name: "banners", description: "Returns old user's banners", type: 1, options: [{ name: "id", description: "User's id", required: true, type: 3 },] };
...
export async function execute(Merlin: Merlin, message: GuildCommandInteraction, args: string[] | null) {
    ...
    return message.createMessage({ content: ((await Merlin.database?.get(`${id}_banners`) || []).map((x: { key: string, date: number}) => `**[Click Here](${x.key})** - <t:${Math.floor(x.date / 1000)}:R>`).join("\n") || "None")})
}