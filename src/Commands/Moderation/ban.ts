...
export const slashCommand: ApplicationCommandOptions = {
    name: "ban", description: "Ban a member", type: 1,
    options: [
...
export async function execute(Merlin: Merlin, message: GuildCommandInteraction, args: string[]): Promise<void> {
    ...
    let reason = message.data.options.getString('reason', false) || "None"
    if (!user) return
    ...
    message.guild.createBan(user.id, { reason: reason }).finally(() => {
        message.createMessage({
            embeds: [{
                title: `<:forge_yes:1169596233338327082> Banned ${user?.username} successfully`,
                color: Merlin.config.color,
                footer: { text: message.guild.name, iconURL: message.guild.iconURL() || undefined },
                timestamp: new Date().toISOString(),
            }]
